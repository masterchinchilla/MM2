...
router.post('/',async (req, res)=>{
    ...
    if(!existingUser){return res.status(401).send({errors:{email: 'Invalid email or password.',
        password: 'Invalid email or password.'}})}else{
            const validPassword= await bcrypt.compare(req.body.password,existingUser.password);
    if(!validPassword){return res.status(400).send({errors:{email: 'Invalid email or password.',
        password: 'Invalid email or password.'}})}else{
